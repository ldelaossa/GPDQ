...
%
% If there is an error in the process, returns GPDQStatus.ERROR or 
% GPDQStatus.CANCELED.
% 
% Usage
% -----
...
function imageSection = createSection(imageName, imageSectionName)
    ...
    % And it does not allow creating multiple sections. 
    if (nargin == 2)
    ...
    % Current mask and image which is shown.
    mask= false(size(image));
    imageRes=[];
    
    % Flag indicating if the mask has been updated since last sving.
    updated = true;
